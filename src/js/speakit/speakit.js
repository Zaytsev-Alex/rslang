...
const speakit = () => {
  ...
  document.querySelector('.intro-btn').addEventListener('click', () => {
    ...
    document.querySelector('.game-speakit').classList.remove('hidden');
  ...
  const isWinGame = () => {
    if (document.querySelectorAll('.guessed-word').length > 9) {
      setTimeout(() => {
        soundIfWinGame();
      }, 1000);
    ...
    if (currentRoundWords.includes(word)) {
      document.querySelectorAll('.item .word').forEach((el) => {
        ...
        if (node.textContent.toLowerCase() === saidWord) {
          ...
          img.src =
            urlData + startScreenWords[node.parentNode.dataset.id].image;
  ...
  const recordBtn = document.querySelector('.btn.records');
